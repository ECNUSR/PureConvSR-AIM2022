    ...
    def topological_transformation(self, qat_path):
        ...
        for layer in model2.layers:
            if layer.name == 'simulation_residual':
                layers2 = layer
        weight = np.zeros((3, 3, config.model['channel'] + 3, 27))
        weight[:, :, :config.model['channel'], :] = layers1.weights[0].numpy()
        bias = layers1.weights[1].numpy()
        for j in range(27):
            weight[1, 1, config.model['channel'] + j % 3, j] = 1
        layers2.weights[0].assign(weight)
        layers2.weights[1].assign(bias)
        self.valid_topological_transformation(model1, model2)
        model2.save('/tmp/mobilesr', overwrite=True, include_optimizer=True, save_format='tf')
        return tf.keras.models.load_model('/tmp/mobilesr', custom_objects={'tf': tf})
