...
from tensorflow.keras.layers import Conv2D, Input, Lambda, Add, Concatenate, ZeroPadding2D
...
def arch(scale, in_channels, out_channels, channel, blocks):
    ...
    x = ZeroPadding2D(blocks // 2)(x)
    for _ in range(blocks):
        x = Conv2D(channel, 2, activation='relu', kernel_initializer=glorot_normal(), bias_initializer='zeros')(x)
