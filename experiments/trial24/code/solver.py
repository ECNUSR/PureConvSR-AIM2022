    ...
    def topological_transformation(self, qat_path):
        ...
        for i in range(goal_step + 1):
            ...
            if i == goal_step:
                ...
                for layer in model1.layers:
                    if layer.name == f'{name}_1_1':
                        layers11 = layer
                    if layer.name == f'{name}_1_2':
                        layers12 = layer
                    if layer.name == f'{name}_2_1':
                        layers21 = layer
                    if layer.name == f'{name}_2_2':
                        layers22 = layer
                weight = np.zeros_like(layers2.weights[0].numpy())
                bias = np.zeros_like(layers2.weights[1].numpy())
                w1 = np.einsum('klio,op->klip', layers11.weights[0].numpy(), layers12.weights[0].numpy()[0, 0])
                w2 = np.einsum('klio,op->klip', layers21.weights[0].numpy(), layers22.weights[0].numpy()[0, 0])
                b1 = np.einsum('o,op->p', layers11.weights[1].numpy(), layers12.weights[0].numpy()[0, 0]) + layers12.weights[1].numpy()
                b2 = np.einsum('o,op->p', layers21.weights[1].numpy(), layers22.weights[0].numpy()[0, 0]) + layers22.weights[1].numpy()
                w = w1 + w2
                k, c = w.shape[1:3]
                for j in range(c):
                    w[k//2,k//2,j,j] += 1
