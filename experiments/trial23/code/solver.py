    ...
    def topological_transformation(self, qat_path):
        ...
        for i in range(goal_step + 1):
            ...
            if i == goal_step:
                ...
                for layer in model1.layers:
                    if layer.name == f'{name}_1':
                        layers11 = layer
                    if layer.name == f'{name}_2':
                        layers12 = layer
                weight = np.zeros_like(layers2.weights[0].numpy())
                bias = np.zeros_like(layers2.weights[1].numpy())
                weight[:, :, :-3, :-3] = np.einsum('klio,op->klip', layers11.weights[0].numpy(), layers12.weights[0].numpy()[0, 0])
                k, c = layers1.weights[0].shape[1:3]
                for j in [-1, -2, -3]:
                    weight[k//2, k//2, j, j] = 1
                bias[:-3] = np.einsum('o,op->p', layers11.weights[1].numpy(), layers12.weights[0].numpy()[0, 0]) + layers12.weights[1].numpy()
