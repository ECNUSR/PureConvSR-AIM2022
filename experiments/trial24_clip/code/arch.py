...
from tensorflow.keras.layers import Conv2D, Input, Lambda, Add, ReLU
from tensorflow.keras.models import Model
from tensorflow.keras.initializers import glorot_normal
import tensorflow.keras.backend as K


def sub_rep(x, channel, name):
    ''' sub rep module '''
    x = Conv2D(channel * 2, 3, padding='same', kernel_initializer=glorot_normal(), bias_initializer='zeros', name=f'{name}_1')(x)
    x = Conv2D(channel, 1, padding='same', kernel_initializer=glorot_normal(), bias_initializer='zeros', name=f'{name}_2')(x)
    return x


def rep(x, channel, name):
    ''' rep module '''
    x1 = sub_rep(x, channel, f'{name}_1')
    x2 = sub_rep(x, channel, f'{name}_2')
    return ReLU()(Add()([x, x1, x2]))
