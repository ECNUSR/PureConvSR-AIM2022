...
def rep_arch(scale, in_channels, out_channels, channel, blocks):
    ...
    x = Conv2D(channel, 5, padding='same', activation='relu', kernel_initializer=glorot_normal(), bias_initializer='zeros')(inp)
    ...
    # Pixel-Shuffle
    x = Concatenate()([x, inp])
    x = Conv2D(out_channels*(scale**2), 5, padding='same', kernel_initializer=glorot_normal(), bias_initializer='zeros', name='simulation_residual')(x)
